...
const CONFIG = {
  ...
  revolutions: 5,
  startRadius: 3.4,           // 5 → 3.4 (daha dar)
  endRadius: 2.1,             // 3.5 → 2.1 (daha dar)
  tileHeightRatio: 0.95,      // 1.1 → 0.95 (daha kısa tile)
  tileSegments: 24,
  spiralGap: 0.29,            // 0.35 → 0.29 (daha sıkı)
  tileOverlap: 0.005,
  cameraZ: 9.5,               // 12 → 9.5 (daha yakın)
  ...
  spiralOffsetY: -1.4,        // -2.0 → -1.4 (biraz yukarı)
};
...
// MOBILE MENU - İYİLEŞTİRİLMİŞ VERSİYON
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (hamburger) {
    ...
    // TÜM LİNKLERE TIKLANINCA MENÜ KAPANSIN
    mobileMenu.addEventListener('click', (e) => {
      if (e.target.tagName === 'A') {
        ...
        // Sayfanın yüklenmesine izin ver (küçük gecikme ile daha güvenli)
        setTimeout(() => {
          // Hiçbir şey yapma, tarayıcı linke gidecek
        }, 50);
      }
    });
    ...
    if (services) {
      services.addEventListener('click', (e) => {
        if (e.target.classList.contains('mobile-services-header')) {
          e.stopImmediatePropagation(); // Services'e tıklayınca menü kapanmasın
